...
describe("cache helpers", () => {
  ...
  it("reads cached values before invoking the fetcher and writes misses to KV", async () => {
    const kv = createKvMock();
    ...
    expect(kv.put).toHaveBeenCalledWith("cache:key", JSON.stringify({ value: 2 }), {
      expirationTtl: 120,
    });
  });

  it("wraps public content fetchers in versioned KV when enabled", async () => {
    const cache = createKvMock();
    const fetcher = vi.fn(async () => ({ value: 2 }));

    setNodeEnv("production");
    cache.get.mockResolvedValueOnce("4");
    cache.get.mockResolvedValueOnce({ value: 1 });

    await expect(
      getPublicContentCached({ CACHE: cache as never }, "home:posts", fetcher, 120),
    ).resolves.toEqual({ value: 1 });

    expect(fetcher).not.toHaveBeenCalled();
    expect(cache.get).toHaveBeenNthCalledWith(1, "cache:version");
    expect(cache.get).toHaveBeenNthCalledWith(2, "home:posts:v4", "json");
  });
