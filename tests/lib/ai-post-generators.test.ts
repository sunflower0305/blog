...
vi.mock("@/lib/ai-provider-profiles", async () => {
  const actual = await vi.importActual<typeof import("@/lib/ai-provider-profiles")>(
    "@/lib/ai-provider-profiles",
  );
  return {
    ...actual,
...
vi.mock("@/lib/ai-image-config", async () => {
  const actual =
    await vi.importActual<typeof import("@/lib/ai-image-config")>("@/lib/ai-image-config");
  return {
    ...actual,
