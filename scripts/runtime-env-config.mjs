...
export function validateRuntimeEnvFiles(contract, { envExamplePath, wranglerPath, packageJsonPath }) {
  ...
  if (actualExample !== expectedExample) {
    errors.push(`${path.relative(repoRoot, envExamplePath)} is not generated from config/runtime-env.json`)
  }
  ...
  for (const [key, expected] of Object.entries(expectedWranglerVars)) {
    if (wranglerVars[key] !== expected) {
      errors.push(`wrangler.toml [vars].${key} must equal contract default ${JSON.stringify(expected)}`)
    }
  }
  ...
  for (const key of Object.keys(wranglerVars)) {
    if (managedKeys.has(key) && !(key in expectedWranglerVars)) {
      errors.push(`wrangler.toml [vars].${key} is managed by the contract but not allowed as a default`)
    }
  }
