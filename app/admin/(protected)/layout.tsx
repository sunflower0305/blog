...
export default async function AdminProtectedLayout({ children }: { children: React.ReactNode }) {
  ...
  const navCls =
    "px-3 py-2 rounded-lg text-sm text-[var(--editor-muted)] hover:text-[var(--editor-ink)] hover:bg-[var(--editor-soft)] transition-all duration-150 whitespace-nowrap";
      ...
      <header className="sticky top-0 z-40 bg-[var(--editor-panel)] border-b border-[var(--editor-line)]">
        <div className="mx-auto max-w-6xl px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-1 overflow-x-auto scrollbar-hide">
            <Link href="/admin/posts" className={navCls}>
              文章
            </Link>
            ...
            <Link href="/admin/settings" className={navCls}>
              设置
            </Link>
