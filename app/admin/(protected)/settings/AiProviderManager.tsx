...
export function AiProviderManager() {
  ...
  const loadProfiles = useCallback(async () => {
    try {
      ...
      setDefaultProfileId(
        typeof data.default_profile_id === "number" ? data.default_profile_id : null,
      );
    } catch {
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (data.success) {
        setTestResult({
          success: true,
          message: `连接成功（${data.model} · ${data.latency_ms}ms）`,
        });
      } else {
    ...
    <div className="space-y-4">
      ...
      {editing && (
        ...
        >
          ...
          <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  updateEditing({ temperature: clampTemperature(Number(e.target.value)) })
                }
          ...
          {testResult ? (
            <div
              className={`mt-3 rounded-lg border px-3 py-2 text-sm ${testResult.success ? "border-emerald-400/40 bg-emerald-500/10 text-[var(--editor-ink)]" : "border-rose-400/40 bg-rose-500/10 text-[var(--editor-ink)]"}`}
            >
              {testResult.success ? "✅ " : "❌ "}
              {testResult.message}
            </div>
