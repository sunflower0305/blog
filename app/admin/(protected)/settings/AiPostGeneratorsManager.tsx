  ...
  return [
    {
      ...
      label: `${normalizedCurrentModel}（当前值）`,
    },
    ...options,
  ];
}
...
export function AiPostGeneratorsManager() {
  ...
  const loadWorkersModels = async (target: GeneratorTarget) => {
    ...
    try {
      ...
      if (data.warning) {
        setWorkersModelsWarning((current) => ({
          ...current,
          [target]: data.warning || "",
        }));
          ...
          >
                  ...
                  {workersModelOptions.length > 0 ? (
                    <div className="mt-2 space-y-2">
                      ...
                      <div className="text-xs text-[var(--editor-muted)]">
                        已加载 {workersModelOptions.length} 个 Workers AI
                        模型。可在下拉里搜索，也可以直接在上方手动输入模型 ID。
                      </div>
                    </div>
                  ) : null}
                  {workersModelsWarning[target] && (
                    <div className="mt-1 text-xs leading-5 text-[var(--editor-muted)]">
                      {workersModelsWarning[target]}
                    </div>
                  )}
                </div>
            ...
            <div className="mt-4 flex flex-col gap-3 text-xs text-[var(--editor-muted)] sm:flex-row sm:items-center sm:justify-between">
              ...
              <button
                ...
                className="inline-flex items-center justify-center rounded-xl bg-[var(--editor-accent)] px-4 py-2 text-sm font-semibold text-[var(--editor-accent-ink)] transition hover:brightness-105 disabled:cursor-not-allowed disabled:opacity-60"
              >
