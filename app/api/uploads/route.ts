...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      !ALL_ALLOWED.includes(file.type) &&
      !file.type.startsWith("image/") &&
      !knownExts.includes(ext)
    ) {
    ...
    if (file.size <= HASH_THRESHOLD) {
      ...
      if (existing) {
        ...
        const dedupUrl = `/api/images/${encodedKey}`;
        const variants =
          category === "image" ? buildAssetUrls(encodedKey, cloudflareImagePipeline) : undefined;
        if (env?.DB) {
    ...
    if (env?.DB) {
      await env.DB.prepare(
        ...
        .bind(key, url, sanitizeFilename(file.name), file.name, contentType, category, file.size)
        .run();
    }
