...
export async function POST(req: NextRequest) {
  try {
    ...
    if (file.size <= HASH_THRESHOLD) {
      ...
      if (existing) {
        ...
        if (env?.DB) {
          ...
          ).bind(dedupKey, dedupUrl, sanitizeFilename(file.name), file.name, file.type, category, file.size).run()
        }
        return NextResponse.json({
          success: true,
          key: dedupKey,
    ...
    if (env?.DB) {
      await env.DB.prepare(
        'INSERT OR IGNORE INTO media (key, url, filename, original_name, file_type, category, size) VALUES (?, ?, ?, ?, ?, ?, ?)'
      ).bind(key, url, sanitizeFilename(file.name), file.name, contentType, category, file.size).run()
    }
