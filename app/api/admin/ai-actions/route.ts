...
export async function POST(req: NextRequest) {
  ...
  const profileId =
    Number.isFinite(body.profile_id) && Number(body.profile_id) > 0
      ? Number(body.profile_id)
      : defaultProfileId;

  // 获取最大 sort_order
  const maxRow = await db
    .prepare("SELECT MAX(sort_order) as max_sort FROM ai_actions")
...
export async function PUT(req: NextRequest) {
  ...
  for (const item of body.items) {
    await db
      .prepare(
        "UPDATE ai_actions SET sort_order = ?, updated_at = strftime('%s', 'now') WHERE id = ?",
      )
