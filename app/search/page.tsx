...
export default async function SearchPage({
  ...
  return (
    <div className="min-h-full flex flex-col bg-[var(--background)]">
      <SiteHeader initialTheme={defaultTheme} navLinks={navLinks} categories={categories} />

      <main className="page-main flex-1 mx-auto max-w-3xl w-full px-4 sm:px-6 py-10 sm:py-14">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-[var(--editor-ink)] mb-2">搜索结果</h1>
          {query ? (
            <p className="text-sm text-[var(--editor-muted)]">
              关键词 &quot;{query}&quot; 找到 {posts.length} 篇文章
            </p>
          ) : (
            <p className="text-sm text-[var(--editor-muted)]">请输入搜索关键词</p>
          )}
        </div>
                    ...
                    >
                      ...
                      {post.password && (
                        <svg
                          ...
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          className="text-[var(--editor-muted)] flex-shrink-0"
                        >
                      ...
                      {post.is_pinned === 1 && (
                        <svg
                          ...
                          strokeLinecap="round"
                          strokeLinejoin="round"
                          className="text-[var(--editor-accent)] flex-shrink-0"
                        >
                        ...
                        <>
                          ...
                          {(() => {
                            const categorySlug = post.category
                              ? categorySlugMap.get(post.category)
                              : null;
                            return categorySlug ? (
                              <Link
