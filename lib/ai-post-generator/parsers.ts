...
export function extractGeneratedTags(parsed: unknown, fallbackText: string) {
  ...
  if (payload && typeof payload === "object") {
    const candidate = readObjectValue(payload as Record<string, unknown>, [
      "tags",
      "keywords",
      "labels",
      "topics",
...
function coerceAssistantText(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    ...
    const nested =
      record.text ?? record.content ?? record.value ?? record.arguments ?? record.output_text;
    if (nested !== undefined) {
...
function extractToolCallArguments(value: unknown): string {
  ...
  if (value && typeof value === "object") {
    ...
    if (record.function && typeof record.function === "object") {
      ...
      const functionArgs = coerceAssistantText(
        functionRecord.arguments ?? functionRecord.content ?? functionRecord.text,
      );
...
export function shouldRetryAssistantPayload(payload: {
  ...
  return (
    ...
    (!!payload.reasoning ||
      ...
      payload.finishReason === "tool_calls")
  );
}
