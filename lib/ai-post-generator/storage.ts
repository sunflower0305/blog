...
async function ensureAiPostGeneratorsTable(db: D1Database) {
  ...
  const tableInfo = await db
    .prepare("PRAGMA table_info(ai_post_generators)")
    .all<{ name: string }>();
  ...
  if (!columns.has("provider_mode")) {
    await db
      .prepare(
        "ALTER TABLE ai_post_generators ADD COLUMN provider_mode TEXT NOT NULL DEFAULT 'workers_ai'",
      )
      .run();
  }
  ...
  if (!columns.has("workers_model")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN workers_model TEXT NOT NULL DEFAULT ''")
      .run();
  }
  if (!columns.has("temperature")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN temperature REAL NOT NULL DEFAULT 0.7")
      .run();
  }
  if (!columns.has("max_tokens")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN max_tokens INTEGER NOT NULL DEFAULT 2000")
      .run();
  }
  if (!columns.has("aspect_ratio")) {
    await db
      .prepare(
        "ALTER TABLE ai_post_generators ADD COLUMN aspect_ratio TEXT NOT NULL DEFAULT '16:9'",
      )
      .run();
  }
  if (!columns.has("resolution")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN resolution TEXT NOT NULL DEFAULT '2k'")
      .run();
  }
  if (!columns.has("is_enabled")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN is_enabled INTEGER NOT NULL DEFAULT 1")
      .run();
  }
  if (!columns.has("is_builtin")) {
    await db
      .prepare("ALTER TABLE ai_post_generators ADD COLUMN is_builtin INTEGER NOT NULL DEFAULT 1")
      .run();
  }
  ...
  for (const seed of DEFAULT_GENERATORS) {
    const existing = await db
      ...
      LIMIT 1
    `)
      .bind(seed.target_key)
      .first<{ id: number; prompt: string; is_builtin: number }>();

    if (existing?.id) {
      const shouldSyncBuiltinPrompt =
...
export async function ensureAiPostGeneratorInfrastructure(
  ...
  if (defaultImageProfileId) {
    ...
    `)
      .bind(defaultImageProfileId)
      .run();
  }
}

export async function listAiPostGenerators(db: D1Database, env?: Partial<CloudflareEnv> | null) {
