...
function collectMatches(value: string, pattern: RegExp, pick: (match: RegExpMatchArray) => string) {
...
function extractSlugFromReasoning(reasoningText: string) {
  ...
  const explicitJson = getLastNonEmpty(
    collectMatches(normalized, /"slug"\s*:\s*"([^"]+)"/gi, (match) => match[1] || ""),
  );
  if (explicitJson) return explicitJson;

  const explicitLine = getLastNonEmpty(
    collectMatches(
      normalized,
...
function extractTagsFromReasoning(reasoningText: string) {
  ...
  const lineCandidate = getLastNonEmpty(
    collectMatches(
      normalized,
...
function collectSlugWords(value: string) {
  const words = collectMatches(value, /[A-Za-z][A-Za-z0-9.+/#-]*/g, (match) => match[0] || "");
...
export function resolveGeneratedSlug(input: MetadataFallbackInput) {
  ...
  return normalizePostSlug(
    `post-${createStableSlugSeed(
      input.title || "",
      input.description || "",
