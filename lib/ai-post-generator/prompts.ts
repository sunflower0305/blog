...
export function buildTextSystemPrompt(target: "summary" | "tags" | "slug", prompt: string) {
  if (target === "summary") {
...
function buildCoverHintPhrases(input: Omit<GeneratePostCoverInput, "db" | "images" | "env">) {
  ...
  return Array.from(
    new Set(
      rawHints.map((item) => item.trim()).filter((item) => item.length >= 2 && item.length <= 24),
    ),
  );
}
...
function buildCoverContentBrief(input: Omit<GeneratePostCoverInput, "db" | "images" | "env">) {
  ...
  for (const sentence of rawSentences.slice(0, 24)) {
    ...
    if (
      targetList.some(
        (item) => item === concise || item.includes(concise) || concise.includes(item),
      )
    ) {
      continue;
    }
...
export function buildCoverPrompt(
  ...
  if (tags.length > 0)
    headSections.push(`标签：${truncateText(tags.join("、"), MAX_COVER_TAGS_LENGTH)}`);
