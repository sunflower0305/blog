...
export function normalizePostSlug(value: string): string {
  return sanitizePostSlugInput(value).replace(/^[-_]+|[-_]+$/g, "");
}
...
const LOCAL_POST_IMAGE_URL =
  /(?:https?:\/\/[^\s"'<>()[\]]+)?\/api\/images\/image\/[^\s"'<>()[\]]+/gi;
...
export function optimizePostImageUrls(value: string, siteUrl: string): string {
  ...
  return value.replace(LOCAL_POST_IMAGE_URL, (input) => {
    ...
    if (
      (isAbsolute && url.origin !== siteOrigin) ||
      !url.pathname.startsWith("/api/images/image/")
    ) {
      return input;
    }
    ...
    const optimized = isAbsolute ? url.toString() : `${url.pathname}${url.search}${url.hash}`;
