...
function readWechatExportStyleTokens(): WechatExportStyleTokens {
  ...
  return {
    ...
    articleQuoteNestedBorderColor: readCssVar(
      rootStyle,
      "--article-quote-nested-border",
      "#b8a68a",
    ),
    articleQuoteNestedBackground: readCssVar(
      rootStyle,
      "--article-quote-nested-bg",
      "rgba(0, 0, 0, 0.02)",
    ),
    bodyFontFamily: bodyFont,
    monoFontFamily: readCssVar(
      rootStyle,
      "--font-geist-mono",
...
export async function copyAsWechatArticleFormat(title: string, html: string) {
  ...
  const { exportedHtml, normalizedTitle } = buildWechatClipboardHtml(title, html);
  const plainText =
    new DOMParser().parseFromString(exportedHtml, "text/html").body.textContent?.trim() ||
    normalizedTitle;
