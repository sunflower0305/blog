...
export function extractWorkersAiModelItems(payload: unknown): RawWorkersAiModelItem[] {
  ...
  return [candidate.result, candidate.data, candidate.items, candidate.models].flatMap((value) =>
    Array.isArray(value) ? (value as RawWorkersAiModelItem[]) : [],
  );
}
  ...
  const filtered = items.filter((item) => {
    ...
    if (typeof item === "string") {
      return kind === "image"
        ...
        : !/(image|text-to-image|diffusion|flux|sdxl|dreamshaper|embedding|bge|rerank|audio|tts|whisper)/.test(
            resolvedId,
          );
    }
    ...
    if (kind === "image") {
      return haystacks.some((value) =>
        /(image generation|text-to-image|image|diffusion|flux|sdxl|dreamshaper|vision)/.test(value),
      );
    }

    return (
      haystacks.some((value) =>
        /(text generation|text-generation|chat|llm|language|reasoning)/.test(value),
      ) ||
      !haystacks.some((value) =>
        /(image|text-to-image|diffusion|flux|sdxl|dreamshaper|embedding|bge|rerank|audio|tts|whisper)/.test(
          value,
