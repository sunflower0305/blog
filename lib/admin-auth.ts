...
export const COOKIE_NAME = "blog_admin";
...
export async function isAdminAuthenticated(cookieValue: string | undefined): Promise<boolean> {
...
/** 生成 API Token（blog_ 前缀 + 32 位 nanoid） */
export function generateApiToken(): string {
  return `blog_${nanoid(32)}`;
}
...
export async function verifyApiToken(db: D1Database, token: string): Promise<boolean> {
  if (!token || !token.startsWith("blog_")) return false;
  try {
    const row = await db
 ...
 */
export async function authenticateRequest(req: NextRequest, db?: D1Database): Promise<boolean> {
  // 1. 先检查 Bearer Token
