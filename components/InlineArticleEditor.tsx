...
export function InlineArticleEditor({
  ...
  const checkDirty = useCallback(
    (
      ...
      const catChanged =
        (overrides?.category ?? categoryValueRef.current) !== originalCategoryRef.current;
      const coverChanged =
        (overrides?.coverImage ?? coverImageValueRef.current) !== originalCoverImageRef.current;
  ...
  return (
    <>
      ...
      >
        ...
        {feedback ? (
          <span
            className={`font-medium ${feedback.type === "success" ? "text-emerald-600" : "text-rose-600"}`}
          >
            {feedback.message}
          </span>
        ) : dirty ? (
          <>
            {charCount > 0 && (
              <span className="text-[var(--editor-line)]" aria-hidden>
                |
              </span>
            )}
            <button
